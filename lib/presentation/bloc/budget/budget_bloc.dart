...
class BudgetBloc extends Bloc<BudgetEvent, BudgetState> {
  ...
  Future<void> _onLoadBudgets(
    ...
    try {
      ...
      final result = await _budgetRepository.getAllBudgets(
        userId: event.userId,
      );
      result.fold(
  ...
  Future<void> _onLoadBudgetsForMonth(
    ...
    try {
      emit(const BudgetState.loading());
      final result = await _budgetRepository.getBudgetsForMonth(
        userId: event.userId,
        month: event.month,
      );
      result.fold(
  ...
  Future<void> _onCreateBudget(
    ...
    try {
      ...
      result.fold((error) => emit(BudgetState.error(message: error)), (budget) {
  ...
  Future<void> _onUpdateBudget(
    ...
    try {
      ...
      result.fold((error) => emit(BudgetState.error(message: error)), (
        updatedBudget,
      ) {
  ...
  Future<void> _onDeleteBudget(
    ...
    try {
      emit(const BudgetState.loading());
      final result = await _budgetRepository.deleteBudget(
        userId: event.userId,
        budgetId: event.budgetId,
      );
      result.fold((error) => emit(BudgetState.error(message: error)), (_) {
